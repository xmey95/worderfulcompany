...
function verify_user_action_authorized(user, requested_username, callback) {
  if (user.id != requested_id) {
    // user requesting action on other user
...
router.get("/", function(req, res) {
  res.json({
    message:
      'Benvenuto alle API della Web App "WonderfulCompany", effettua una richiesta valida :)'
  });
});
...
router
  .route("/authenticate")
  .get(auth.isAuthenticatedBasic, function(req, res) {
...
router.route("/users").post(function(req, res) {
  pool.getConnection(function(err, connection) {
    ...
    connection.query("INSERT INTO ?? SET ?", ["users", post], function(
      err,
      results,
      fields
    ) {
  ...
  pool.getConnection(function(err, connection) {
    ...
    connection.query("SELECT * FROM ??", ["users"], function(
      ...
      if (!results)
        return res
          .status(404)
          .send(JSON.stringify({ success: false, error: "USERS_NOT_FOUND" }));
      ...
      if (req.params.version == "false") {
        return res
          .status(201)
          .send(JSON.stringify({ success: true, error: null, users: users }));
      } else {
...
router.route("/supervisions/:version").get(function(req, res) {
  pool.getConnection(function(err, connection) {
    // Use the connection
    connection.query("SELECT * FROM ??", ["supervisions"], function(
      err,
      results,
      fields
    ) {
      ...
      if (!results)
        return res
          .status(404)
          .send(
            JSON.stringify({ success: false, error: "SUPERVISIONS_NOT_FOUND" })
          );
    ...
    connection.query(
      ...
      function(err, results, fields) {
        ...
        if (!results[0])
          return res
            .status(404)
            .send(JSON.stringify({ success: false, error: "USER_NOT_FOUND" }));
...
router
  .route("/users/set_survey_manager/:user/:survey")
  .get(auth.isAuthenticated, function(req, res) {
    pool.getConnection(function(err, connection) {
      connection.query(
        "UPDATE ?? SET ?? = ? WHERE ?? = ?",
        ["surveys", "id_user", req.body.user, "id", req.body.survey],
        function(err, results, fields) {
          connection.release();
...
router.route("/users/set_boss/").post(function(req, res) {
  pool.getConnection(function(err, connection) {
    connection.query(
      "SELECT * FROM ?? WHERE ?? = ?",
      ["supervisions", "id_user", req.body.user],
      function(err, results, fields) {
        if (err)
          return res
            .status(500)
        ...
        if (!results)
          return res
            ...
            .send(JSON.stringify({ success: false, error: "USER_NOT_FOUND" }));
        if (results.length == 0) {
          //l'utente non aveva un boss, inserimento nuova entry nella tabella supervisions
          ...
          //cambio del boss, se già l'utente ne aveva uno
          connection.query(
            "UPDATE ?? SET ?? = ? WHERE ?? = ?",
            [
              "supervisions",
...
router.route("/users/:user/get_boss/").get(function(req, res) {
  pool.getConnection(function(err, connection) {
    connection.query(
      "SELECT ?? FROM ?? WHERE ?? = ?",
      ["id_boss", "supervisions", "id_user", req.params.user],
      function(err, results, fields) {
        connection.release();
        if (err)
          return res
        ...
        if (!results || results.length == 0)
          return res
            .status(404)
            .send(JSON.stringify({ success: false, error: "BOSS_NOT_FOUND" }));
        res
          .status(201)
          .send(
            JSON.stringify({
    ...
    connection.query(
      ...
      function(err, results, fields) {
        ...
        res
          .status(201)
          .send(
            JSON.stringify({ success: true, error: null, user: results[0].id })
          );
      }
    );
  });

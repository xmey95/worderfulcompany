...
router.get("/", function(req, res) {
  res.json({
    message:
      'Benvenuto alle API della Web App "WonderfulCompany"(sezione aulario), effettua una richiesta valida :)'
  });
});
...
router.route("/rooms").post(auth.isAuthenticated, function(req, res) {
  pool.getConnection(function(err, connection) {
    // Use the connection
    var accessories = [];
    if (req.body.accessories) accessories = JSON.parse(req.body.accessories);
    ...
    connection.query("INSERT INTO ?? SET ?", ["rooms", post], function(
      err,
      results,
      fields
    ) {
      ...
      for (var i = 0; i < accessories.length; i++) {
        var post = {
          id_room: results.insertId,
          id_accessory: accessories[i].id_accessory,
          quantity: accessories[i].quantity
        };
        connection.query(
          "INSERT INTO ?? SET ?",
          ["accessoriesrooms", post],
...
router.route("/rooms/:version").get(auth.isAuthenticated, function(req, res) {
  pool.getConnection(function(err, connection) {
    // Use the connection
    connection.query("SELECT * FROM ??", ["rooms"], function(
      err,
      results,
      fields
    ) {
      ...
      if (!results)
        return res
          .status(404)
          .send(JSON.stringify({ success: false, error: "ROOMS_NOT_FOUND" }));
      var rooms = results;

      connection.query("SELECT * FROM ??", ["accessoriesrooms"], function(
        err,
        results,
        fields
      ) {
        ...
        if (req.params.version == "false") {
          return res
            .status(200)
            .send(JSON.stringify({ success: true, error: null, rooms: rooms }));
        } else {
...
router.route("/accessories").post(auth.isAuthenticated, function(req, res) {
  pool.getConnection(function(err, connection) {
    ...
    connection.query("INSERT INTO ?? SET ?", ["accessories", post], function(
      err,
      results,
      fields
    ) {
        ...
        function(err, results, fields) {
          ...
          if (!results || results.length == 0) {
            ...
            return res
              .status(404)
              .send(
                JSON.stringify({ success: false, error: "ROOM_NOT_FOUND" })
              );
          }
    ...
    pool.getConnection(function(err, connection) {
      ...
      connection.query(
        ...
        function(err, results, fields) {
          ...
          connection.query(
            "SELECT * FROM ?? WHERE ?? = ?",
            ["accessoriesrooms", "id_room", id_room],
            function(err, results, fields) {
              connection.release();
              if (err)
                return res
              ...
              if (!results)
                return res
                  .status(404)
                  .send(
                    JSON.stringify({ success: false, error: "ROOMS_NOT_FOUND" })
                  );
    ...
    pool.getConnection(function(err, connection) {
      ...
      connection.query(
        ...
        function(err, results, fields) {
          ...
          var post = {
            id_room: id_room,
            id_accessory: req.body.id_accessory,
...
router
  .route("/rooms/:room/accessories/:accessory")
  .delete(auth.isAuthenticated, function(req, res) {
    pool.getConnection(function(err, connection) {
      // Use the connection
      var id_room = req.params.room;

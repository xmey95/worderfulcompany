...
export class RequestChartsComponent implements OnChanges {
  ...
  colorScheme = { //color scheme for charts
    domain: ['#a8385d', '#7aa3e5', '#a27ea8', '#aae3f5', '#adcded', '#a95963', '#AAAAAA']
  };
  userscolorScheme = { //color scheme for user absence chart
  ...
  days_in_month(request,month){
    ...
    if(start == end){ //one day absence
      if(this.year_filter==true && start.getFullYear() != this.year_selected)return 0; //year filter
      if(start.getMonth == month)return 1;
      else return 0;
    }

    if(start.getFullYear() == end.getFullYear()){ //the absence starts and ends in the same year
      if(this.year_filter==true && start.getFullYear() != this.year_selected)return 0; //year filter


      if (start.getMonth()<month && end.getMonth() > month) return this.month_days(month); //the whole month is in the request period

      if(start.getMonth() < month && end.getMonth() == month){ //the absence starts before and ends in the specified month
      ...
      if(start.getMonth() == month && end.getMonth() > month){ //the absence starts during the month and ends after it
      ...
      if(start.getMonth() == month && end.getMonth() == month){ //The absence in entirely contained in the month
    ...
    else{ //The request starts in a certain year and ends inthe year after it
      if(start.getMonth > month && end.getMonth < month)return 0; //no day of absence in the month

      if(month > start.getMonth()){ //The month is after the start of the request but before the end of the year
        if(this.year_filter == true && start.getFullYear()!=this.year_selected)return 0; //year filter
        return this.month_days(month);
      }
      if(month < end.getMonth()){ //The month is in the year after the start but the end of the request is after it
        if(this.year_filter == true && end.getFullYear()!=this.year_selected)return 0; //year filter
        return this.month_days(month);
      }

      if(start.getMonth() == month){ //The absence starts during the month and ends in the year later
        if(this.year_filter == true && start.getFullYear()!=this.year_selected)return 0; //year filter
      ...
      if(end.getMonth()==month){ //The requests starts in the past year and ends during the month
  ...
  days_in_selected_year(start_date, end_date){
    ...
    if(start.getFullYear() == end.getFullYear()){ //the absence starts and ends in the same year
    ...
    }else{ //the request starts in a certain year and ends in the later one
      if(this.year_selected == start.getFullYear()){ //start year is selected
      ...
      if(this.year_selected == end.getFullYear()){ //end year is selected
  ...
  get_absence_days(id){
    ...
    for(var i = 0; i < this.requests.length; i++){
      if(this.requests[i].id_user == id){
        if((this.requests[i].state == 0 && this.consider_pending == false) || this.requests[i].state == 2 ) continue; //state filter
        if(this.year_filter == false){
          count = count + this.absence_len(this.requests[i].start_date, this.requests[i].end_date);
        }else{
          count = count + this.days_in_selected_year(this.requests[i].start_date, this.requests[i].end_date);
        }
      }
    }
    return count;
  ...
  get_monthly_absences_with_reason(month,reason){
    ...
    for (var i = 0; i < this.requests.length; i++){
      if((this.requests[i].state == 0 && this.consider_pending == false) || this.requests[i].state == 2 ) continue; //state filter
      if(this.requests[i].reason != "malattia" && this.requests[i].reason != "studio" && this.requests[i].reason != "ferie"){ //check reason
  ...
  make_cards_data(){
    ...
    if(this.general){ //general overview
      return [
        {
          "name": "Richieste",
          "value": req_number
  ...
  make_pie_data(){
    ...
    if(this.requests){
      for(var i = 0; i < this.requests.length; i++){
        if(this.year_filter == true && this.check_year_belonging(this.requests[i],this.year_selected) == false) continue; //year filter
        if(this.requests[i].state == 2 || (this.consider_pending == false && this.requests[i].state == 0)) continue; //state filter
